...
const CartWidget = () => {
  ...
  return (
    <>
      <button onClick={toggleCartVisibility} className="cart-button relative">
        <CartIcon />
        {cart.length > 0 && (
          <span className="absolute -top-3 -right-3 bg-red-500 text-white rounded-full text-xs px-2 py-1 flex items-center justify-center h-6 w-6 z-10">
            {getItemCount()}
          </span>
        )}
      </button>
      ...
      {isCartVisible && (
        <aside className="cart flex flex-col h-full">
          ...
          <button
            ...
            className="mt-auto bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mb-4 self-center"
          >
            Finalizar Compra
          </button>
          {orderInfo.completed && (
